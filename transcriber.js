/**
 * =============================================================================
 * Dictionario IALA - Transcriptor Fonetic e Phonetic Rules pro Interlingua
 * =============================================================================
 * 
...
(function(global) {
  ...
  // §1: Le litteras e lor nomines
  const iaLetterName = function(c) {
  ...
  // Átomos numéricos básicos para Interlingua
  ...
  // Conversión recursiva de enteros a texto en Interlingua
  ...
  const preprocessInterlinguaText = function(text) {
    if (!text) return '';
    // Strip markup & formatting
    let clean = text
      .replace(/##[^#]+##/g, '')
      .replace(/\*\*([^*]+)\*\*/g, '$1')
      .replace(/_([^_]+)_/g, '$1')
    ...
    // Expandir letras individuales aisladas
    const letterRe = /\b([A-Za-z])\b/g;
    textWithNums = textWithNums.replace(letterRe, function(match, letter) {
  ...
  // Transcribir palabra individual a IPA estricto según fonética IALA / UMI
  const transcribeWordInterlingua = function(word) {
    const lowerWord = (word || '').toLowerCase();
    const chars = Array.from(lowerWord);

    if (chars.length === 0) return '';

    const isVowel = function(c) { return 'aeiouy'.includes(c); };
    const vowelIndices = [];
    ...
    while (charIdx < chars.length) {
      ...
      switch (char) {
        ...
        case 's': {
          ...
          // (o, optionalmente, como -s in anglese {these}).
          // Le norma primordial e general es /s/ surda.
  ...
  const transcribeInterlinguaToSsml = function(text) {
  ...
  const getInterlinguaIpaString = function(text) {
    const delimiters = " ,.?!;:\u201C\u201D\u00AB\u00BB()'-";
    const parts = [];
    let last = 0;
  ...
  const InterlinguaTranscriber = {
    transcribeInterlinguaToSsml: transcribeInterlinguaToSsml,
    getInterlinguaIpaString: getInterlinguaIpaString,
    transcribeWordInterlingua: transcribeWordInterlingua,
    iaLetterName: iaLetterName,
    numAtomInterlingua: numAtomInterlingua,
    transcribeIntInterlingua: transcribeIntInterlingua,
    preprocessInterlinguaText: preprocessInterlinguaText
  };

  if (typeof module !== 'undefined' && module.exports) {
    module.exports = InterlinguaTranscriber;
  }
  global.InterlinguaTranscriber = InterlinguaTranscriber;
